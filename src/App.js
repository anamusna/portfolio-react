...
class App extends Component {
	state = {
		...
		homeName         : 'Ansumana',
		job              : 'JavaScript | React.js | Node.js | Redux | MongoDB',
		email            : 'darboe.d@yahoo.com',
		bio              : 'Berlin',
		menuBar          : [
	...
	fetchAsync = async () => {
		try {
			const result = await fetch(
				`https://api.github.com/users/${this.state.githubData
					.githubName}?client_id= 6e5cd7b5f80da5576dd742cfbee01ad0e5727cc6&client_secret=${key}`
			);
	...
	whatToRender = () => {
		...
		switch (this.state.menu) {
			...
			case 'Projects':
				...
				break;
			case 'Contact':
				whatToRender = <Contact email={this.state.email} socialMedia={[ ...this.state.socialMedia ]} />;
				break;
			default:
				whatToRender = (
					<Home
